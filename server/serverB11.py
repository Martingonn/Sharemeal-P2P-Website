...
def update():
    ...
    for peer_id, peer_data in peers.items():
        try:
            requests.post(
                f"{peer_data['address']}/update_server",
                json={"file_url": f"http://{request.host}/get_update/{file.filename}"}
            )
        except Exception as e:
            print(f"Failed to notify peer {peer_id} at {peer_data['address']}: {e}")
